...
List<NavGroup> navGroupsFor({
  ...
  Set<String> modules = const {'inventory', 'pos', 'customers', 'reports'},
      ...
      // المرتَّبات والسلف لإصدارات المحفظة **أو لمن اشترى وحدة البطاقات**:
      // جهةٌ تصرف على منتسبيها تحتاجهما، ومتجرٌ يبيع بضاعة لا معنى لهما في
      // قائمته — إلا أن يكون محلاً عسكرياً أو جمعيةً تصرف على أعضائها،
      // فيبيع البضاعة **ويصرف** معاً. والشرطان معاً لا أحدهما: إصدار
      // المحفظة البسيطة يصرف مرتَّبات بلا أن يشتري الوحدة، وإسقاطُ شقّه
      // يسلبه شاشةً يملكها اليوم.
      if (_isWalletShaped(edition) || modules.contains('wallet'))
        const NavGroup(
