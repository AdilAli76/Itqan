...
class ApiClient {
  ...
  /// المكتب يسقطان على <c>localhost:5000</c> — أي أن الهاتف يكلّم نفسه فلا يدخل
  ...
  ///
  /// `local://localhost:5000` قيمة خاصة تعني استخدام قاعدة البيانات المحلية فقط.
  static String normalizeServerUrl(String raw) {
    var url = raw.trim();
    if (url.isEmpty) return '';

    // قيمة خاصة للقاعدة المحلية
    if (url == 'local://localhost:5000') {
      return 'local://localhost:5000';
    }
