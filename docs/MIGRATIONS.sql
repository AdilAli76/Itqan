...
-- ----------------------------------------------------------------------------
--  كلمةٌ مؤقّتة تنتظر التغيير
--
--  إعادة التعيين تُولّد كلمةً تُملى هاتفياً وتُسلَّم «ليغيّرها» — ولم يكن في
--  النظام كلّه نقطةٌ يغيّر بها المستخدم كلمته أصلاً. فتبقى الكلمة التي
--  أملاها مشغّل النظام هي كلمة الحساب الدائمة، ويعرفها اثنان.
--
--  والراية تُرفع عند كل إعادة تعيين من **غير** صاحب الحساب، وتُخفض حين
--  يغيّرها هو. والفرض على الخادم لا في الشاشة — راجع MustChangePasswordFilter.
--
--  والافتراضي صفر: الحسابات القائمة لم تُعَد كلماتها، وترقيتها إلى 1 كانت
--  تُغلق النظام على كل مستخدميه صباح يوم النشر.
-- ----------------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sys.columns
               WHERE object_id = OBJECT_ID('dbo.app_users') AND name = 'must_change_password')
BEGIN
    ALTER TABLE dbo.app_users ADD must_change_password BIT NOT NULL
        CONSTRAINT DF_app_users_must_change_password DEFAULT 0;
    PRINT N'أُضيف عمود app_users.must_change_password';
END
GO

-- ----------------------------------------------------------------------------
--  صلاحية النسخة الاحتياطية
--
--  النسخة تُنزَّل بصلاحيةٍ يمنحها مالك المنظمة، لا بدورٍ ثابت في الكود:
--  جهةٌ محاسبها هو من يحفظ نسخها الأسبوعية لا يجوز أن تنتظر مالكها كلّ
--  مرّة، وأخرى ترى الملف أخطر من أن يخرج من يد المالك. والفرق بينهما قرارُ
--  المالك من شاشة «مصفوفة الصلاحيات» لا قرارُنا.
--
--  ولا تُمنَح لأي دور افتراضاً: الملف يحوي كل بيانات المنظمة وبصمات كلمات
--  المرور، فمن يريد منحه يمنحه صراحةً. وsuper_admin يتجاوز الفحص دائماً
--  فلا يُغلَق الباب على المالك نفسه.
-- ----------------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM dbo.permissions WHERE code = 'backup.manage')
BEGIN
    INSERT INTO dbo.permissions (code, label_ar, module)
    VALUES ('backup.manage', N'تنزيل نسخة احتياطية من بيانات المنظمة', N'backup');
    PRINT N'أُضيفت صلاحية backup.manage';
END
GO

-- ----------------------------------------------------------------------------
--  النسخة الاحتياطية التلقائية إلى Google Drive
--
--  النسخة اليدوية تعتمد على أن يتذكّرها إنسان — يتذكّرها شهراً ثم ينسى،
--  ولا يكتشف نسيانه إلا يوم يحتاجها.
--
--  والساعة بتوقيت المنظمة لا بـUTC (راجع OrgClock): «الثالثة فجراً» عند
--  صاحب المحلّ تعني الثالثة عنده، وضبطها بغرينتش كان يضع الرفع في ذروة
--  البيع عند بعض المناطق.
--
--  ورمز التحديث سرٌّ كامل: من يملكه يكتب في درايف صاحبه. لا يخرج في أي
--  استجابة، ولا يُسجَّل في سجلّ التدقيق.
-- ----------------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sys.columns
               WHERE object_id = OBJECT_ID('dbo.organizations') AND name = 'auto_backup_enabled')
BEGIN
    ALTER TABLE dbo.organizations ADD
        auto_backup_enabled BIT NOT NULL CONSTRAINT DF_org_auto_backup_enabled DEFAULT 0,
        auto_backup_hour INT NOT NULL CONSTRAINT DF_org_auto_backup_hour DEFAULT 3,
    ...
    PRINT N'أُضيفت أعمدة النسخة الاحتياطية التلقائية';
END
GO
...
-- ----------------------------------------------------------------------------
--  طريقة تكلفة الصنف — قرارُ إدارة
--
--  صنفٌ في مخزنه شحنتان بسعرين، وبطاقتُه تعرض رقماً واحداً يقيس عليه الحدّ
--  الأدنى للسعر ويُعرض به الهامش. فأيّهما يُقال: آخر شراء، أم متوسط ما في
--  المخزن، أم تكلفة الشحنة التي ستخرج في البيع التالي؟
--
--  والافتراضي 'last_purchase' — سلوك النظام قبل هذا العمود، فلا يتغيّر شيء
--  عند أحد ما لم يختره بنفسه. ولا يمسّ أيٌّ منها تكلفة البضاعة المباعة:
--  تلك تُقرأ من الدفعة التي خرجت منها فعلاً.
-- ----------------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sys.columns
               WHERE object_id = OBJECT_ID('dbo.organizations') AND name = 'inventory_costing_method')
BEGIN
    ALTER TABLE dbo.organizations ADD inventory_costing_method NVARCHAR(30) NOT NULL
        CONSTRAINT DF_organizations_inventory_costing DEFAULT N'last_purchase';
    PRINT N'أُضيف عمود organizations.inventory_costing_method';
END
GO
