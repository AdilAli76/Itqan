...
public class Organization
{
    ...
    /// <summary>
    /// ما يعنيه «تكلفة الصنف» في هذه المنظمة — راجع [CostingMethods].
    ///
    /// <para><b>ولا يمسّ ربحك المحسوب:</b> تكلفة البضاعة المباعة تُقرأ من
    /// الدفعة التي خرجت منها البضاعة فعلاً (راجع [StockLedger.IssueAsync])
    /// ولا تتغيّر بهذا الإعداد. وإنما يضبط الرقم المرجعيّ الواحد على بطاقة
    /// الصنف: ما يقيس عليه الحدّ الأدنى للسعر ويُعرض به الهامش.</para>
    /// </summary>
    public string InventoryCostingMethod { get; set; } = CostingMethods.LastPurchase;
...
/// <summary>
/// ما يعنيه «تكلفة الصنف» على بطاقته — قرارُ إدارةٍ لا قاعدةٌ واحدة.
///
/// <para><b>ولا يمسّ أيٌّ منها الربح المحسوب:</b> تكلفة البضاعة المباعة
/// تُقرأ من الدفعة التي خرجت منها فعلاً، دفعةً دفعة. وهذه الثلاثة إنما
/// تضبط الرقم المرجعيّ الذي يقيس عليه الحدّ الأدنى للسعر ويُعرض به
/// الهامش — رقمٌ واحد لصنفٍ في مخزنه شحنتان بسعرين، فلا بدّ من اختيار
/// أيّهما يُقال.</para>
/// </summary>
public static class CostingMethods
{
    /// <summary>
    /// آخر شراء — التكلفة هي سعر آخر شحنة وصلت، محمَّلةً بمصاريفها.
    ///
    /// <para>الافتراضي وسلوك النظام قبل هذا الإعداد. ومعناه العملي:
    /// «بكم أشتريه اليوم» — وهو ما يسأل عنه التاجر حين يسعّر. وعيبُه أن
    /// شراءً صغيراً بسعرٍ مرتفع يرفع الرقم المرجعيّ لمخزونٍ قديم كلّه.</para>
    /// </summary>
    public const string LastPurchase = "last_purchase";

    /// <summary>
    /// المتوسط المرجّح — قيمة ما في المخزن كلّه مقسومةً على كميّته.
    ///
    /// <para>لمن يبيع بضاعةً متماثلة من شحناتٍ مختلطة: الرقم يمثّل ما في
    /// المخزن فعلاً لا آخر صفقة. وعيبُه أنه لا يساوي سعر أي شحنة بعينها،
    /// فمن يسعّر عليه قد يبيع شحنةً غالية بربح شحنةٍ رخيصة.</para>
    /// </summary>
    public const string WeightedAverage = "weighted_average";

    /// <summary>
    /// تكلفة الدفعة — تكلفة الشحنة التي ستخرج في البيع التالي.
    ///
    /// <para>الأقرب إلى الحقيقة: هو بالضبط ما ستُحمَّل به الفاتورة التالية،
    /// لأن الصرف يأخذ من الأقدم صلاحيةً (FEFO) أو دخولاً (FIFO). ولمن
    /// يتتبّع الصلاحية هو الوحيد الذي لا يكذب — دواءٌ من شحنةٍ غالية قارب
    /// انتهاؤه لا يُسعَّر بتكلفة شحنةٍ رخيصة لم تُفتح بعد.</para>
    /// </summary>
    public const string Batch = "batch";

    public static readonly string[] All = { LastPurchase, WeightedAverage, Batch };
