...
if ($canManageInventory -and $accProduct -and $accProduct.Status -in 200,201) {
    ...
    if ($po.Status -in 200,201) {
        ...
        if ($recvEntry) {
            # ⚠ الاستلام يُقيَّد على «وردت ولم تُفوتَر» (2104) لا على
            # «الموردون» (2101).
            #
            # كان هذا الفحص يتوقّع 2101 قبل أن يُفصَل الاستلام عن الفوترة.
            # والفرق ليس تصنيفاً: بضاعة وصلت ولم تصل فاتورتها **ليست ديناً
            # بمبلغٍ متّفق عليه** — سعرها المفترَض سعر الطلبية، وقد تأتي
            # الفاتورة بغيره. فقيدُها على الموردين يُثبت رقماً لم يقُله
            # المورّد بعد.
            #
            # وفاتورة المورّد هي التي تُفرّغ 2104 إلى 2101 بالمبلغ الحقيقي —
            # يفحصه القسم ٥.٦.
            $usesGrni = $recvEntry.lines | Where-Object { $_.accountCode -eq '2104' -and $_.credit -gt 0 }
            Check "الاستلام يُقيَّد على «وردت ولم تُفوتَر» لا على الموردين" ($null -ne $usesGrni) `
                "قيدُه على الموردين يُثبت ديناً بسعرٍ لم تقُله فاتورة المورّد بعد"
        }
...
if ($sup.Status -notin 200,201) {
    ...
    # ‏balance لا walletBalance: الثاني حقل العميل، وكشف المورّد يحمل
    # الأوّل. وقراءة اسمٍ غير موجود تُرجع فارغاً لا خطأً — فتُقاس صفراً.
